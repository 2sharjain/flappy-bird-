...
def gameOver(score):
    canvas.destroy()
    canvas1=Canvas(root,width = 400,height = 500,bg = "WHITE",bd = 0, highlightthickness = 0)
    canvas1.create_text(200,200,text="GAME OVER",font=("Comic San MS",50,"bold"))
    canvas1.create_text(200,250,text=("YOUR SCORE IS : "+str(score)),font=("Comic San MS",20,"bold"))
    canvas1.pack()
    root.update()
    root.mainloop()
class ball:
...
class Rectangle:
    ...
    def move(self):
        coords = self.canvas.coords(self.id1)
        coords = self.canvas.coords(self.id2)
